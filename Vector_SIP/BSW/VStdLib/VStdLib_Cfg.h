 ...
 *
 * If set to STD_ON all memory manipulation routines are mapped to external functions (e.g. compiler libraries or
 * other implementations that are optimized for the target platform). As the generic VStdLib implementation provides
 * optimized routines only for 32bit platforms, this is recommended when using MCUs with other architectures.
 *
 * Values:
