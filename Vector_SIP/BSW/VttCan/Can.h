...
| 2017-06-07 5.07.00   Bir   - FEAT-2738: support silent mode
|                            - FEAT-2888: selective MultiplexedTx
| 2018-02-01 5.08.00   Bir   - Support configuration where BasicCANRxSupport switch is active without BasicCANRx configured
|                            - STORYC-3057: support Mirror Mode with FD 
| 2018-04-24 6.00.00   Bir   - Update to RI2.0 for MSR
|                            - STORYC-6328: support HwRxFIFO (configurable size) 
|                            - ESCAN00101526: do not call GetElapsedValue() to avoid DET
|                            - STORYC-6324: Support emulation of one Infixed CAN driver
...
#define DRVCAN_CANOEEMUCANOEASR_VERSION 0x0600u
...
/* as workaround to support OS timer function also for VTT */
#define UnusedCounter 0u
#define OS_TICKS2MS_UnusedCounter(tick) tick

#define kCanIdTypeStd                 (0x00000000UL)
#define kCanIdTypeExt                 (0x80000000UL)
#define kCanIdTypeExtMaskOut          (0x7FFFFFFFUL)
/* return values of CanRxActualFdType */

#if defined(C_ENABLE_EXTENDED_ID)
# define kCanFdTypeDef                 vuint32
# define kCanFdTypeFd                  (0x40000000UL) /* get from CANoeApi EDL flag but written to CanId so set to second highest ID bit */
# define kCanFdBrsTypeTrue             (0x20000000UL)
# define kCanFdTypeFdMaskOut           (0x9FFFFFFFUL)
#else
# define kCanFdTypeDef                 vuint16
# define kCanFdTypeFd                  (0x4000u)
# define kCanFdBrsTypeTrue             (0x2000u)
# define kCanFdTypeFdMaskOut           (0x1FFFu)
#endif
#define kCanFdTypeClassic              (0x0000u)

#define kCanFdBrsTypeFalse             (0x0000u)
...
#define kCanVoltageNormal  0u
#define kCanVoltageHigh    1u

#define CAN_MODE_INTER  ((vuint8)0u) /* Intermediate state - used to simulate the hw-loop */
#define CAN_MODE_STOP   ((vuint8)1u)
#define CAN_MODE_START  ((vuint8)2u)
#define CAN_MODE_SLEEP  ((vuint8)3u)
#define CAN_MODE_UNINIT ((vuint8)4u)
...
#define CAN_MSGOBJ_TX      ((vuint8)0u)
#define CAN_MSGOBJ_RXFULL  ((vuint8)1u)
#define CAN_MSGOBJ_RXBASIC ((vuint8)2u)
#define CAN_MSGOBJ_UNUSED  ((vuint8)3u)

#define kCanLoopInit       ((vuint8)0u)
#define kCanLoopRx         ((vuint8)1u)
...
typedef kCanFdTypeDef tCanFdType;
typedef kCanFdTypeDef tCanFdBrsType;
#endif
...
V_DEF_P2SFR_CAN_TYPE(volatile, Can_MsgDataStructType)  CanChipMsgPtr;
 ...
 *  Can_RxIndication()
 ***********************************************************************************************************/
/*! \brief         RX indication handling
 *  \details       Receiving message data from CANoe by copy received data into a emulated CAN cell buffer.
 *  \param[in]     canChannel            CAN controller
 ...
 *  \config        -
***********************************************************************************************************/
V_DEF_FUNC(V_NONE, void, CODE) Can_RxIndication(vuint8 canChannel, vuint32 id, vuint32 flags, vuint8 dataLength, const vuint8* data);
/* CODE CATEGORY 1 END */
 ...
 *  \details       handle TX confirmation from CANoe by mark emulated CAN cell mailbox as be confirmed.
 *  \param[in]     channel               CAN controller
 *  \param[in]     id                    ID of the transmitted message
 *  \pre           -
 *  \context       ANY
 *  \reentrant     FALSE
 *  \synchronous   TRUE
 *  \config        !defined(CAN_VTT_DRIVER)
***********************************************************************************************************/
V_DEF_FUNC(V_NONE, void, CODE) Can_TxConfirmation(vuint8 canChannel, vuint32 id, vuint32 flags, vuint8 dataLength, const vuint8* data);
/* CODE CATEGORY 1 END */
 ...
 *  \details       handle the BUSOFF indication from CANoe
 *  \param[in]     channel               CAN controller
 *  \param[in]     errCode               error code specify the event
 *  \param[in]     errCntTx              TX error counter value
 *  \param[in]     errCntRx              RX error counter value
 *  \pre           -
 *  \context       ANY
 *  \reentrant     FALSE
 *  \synchronous   TRUE
***********************************************************************************************************/
V_DEF_FUNC(V_NONE, void, CODE) Can_ErrIndication(vuint8 canChannel, vuint32 errCode, vuint8 errCntTx, vuint8 errCntRx);
/* CODE CATEGORY 1 END */
