...
{
  if(Os_CoreIsSingleCoreOs() == 0u)                                                                                     /* COV_OS_TESTSUITERESTRICTIONMULTICORE */
  {
...
)                                                                                                                       /* COV_OS_UNSUPPORTED93797 */
{
  ...
  if(Os_BarrierIsEnabled() == 0u)                                                                                       /* PRQA S 2992, 2996 */ /* MD_Os_Rule14.3_2992, MD_Os_Rule2.2_2996 */
  {
    status = Os_ErrSetOkInStdStatus(OS_STATUS_ID_1);                                                                    /* PRQA S 2880 */ /* MD_Os_Rule2.1_2880 */
  }
  else if(OS_UNLIKELY(Os_ThreadCheckCallContext(currentThread, OS_APICONTEXT_BARRIERSYNCHRONIZE) == OS_CHECK_FAILED))   /* SBSW_OS_BR_THREADCHECKCALLCONTEXT_001 */
  {
