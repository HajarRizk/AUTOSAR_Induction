...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  Timer->Dyn->LastCounterValue = Os_TimerFrtTickType2TickType(timeZero);          /* store current counter value.   */  /* SBSW_OS_TIMER_TIMERGETDYN_001 */
 ...
 *
 * */
  return (Timer->Dyn->HwTimeAtLogicalZero + Os_TimerTickType2FrtTickType(Value));
}
...
{
  Os_TickType now;
  Os_Hal_TimerFrtTickType correctedExpirationTime;

  /* #10 Store the ExpirationTime for the case that the compare difference does not fit into the hardware part
   *     of the counter. */
  Timer->Dyn->Compare = ExpirationTime;                                                                                 /* SBSW_OS_TIMER_TIMERGETDYN_001 */
  correctedExpirationTime = Os_TimerHrtTickType2FrtTickType(Timer, ExpirationTime);                                     /* SBSW_OS_FC_PRECONDITION */

  /* #20 Set the compare value. */
  Os_TimerFrtSetCompareValue(Timer->HwConfig, Os_TimerFrtTickType2TickType(correctedExpirationTime));                   /* SBSW_OS_TIMER_TIMERFRTSETCOMPAREVALUE_001 */ /* SBSW_OS_FC_PRECONDITION */
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  timePassed = (now - Os_TimerFrtTickType2TickType(Timer->Dyn->LastCounterValue));
...
))
{
  ...
  Os_Assert((Os_StdReturnType)(Value <= MaxCountingValue));                                                             /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */ /* COMP_WARN_OS_CW_005 */
  Os_Assert((Os_StdReturnType)(Add <=  MaxAllowedValue));                                                               /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */ /* COMP_WARN_OS_CW_005 */
...
))                                                                                                                      /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  ...
  if(Os_TimerIsFrtCorrectionNeeded() != 0u)                                                                             /* COV_OS_HALPLATFORMFRT64BITAVAILABILITY */
  {
    Os_Hal_TimerFrtTickType now;      /* current time in (big) HAL type */
    Os_TickType             shortNow; /* current time in (smaller) Core type */
    Os_Hal_TimerFrtTickType compareValue;
    ...
    /* #30 Calculate the compare value by transferring the upper bits of now value to it. */
    compareValue = Os_TimerFrtGetFrtTickTypeUpperBits(now) + CompareValue;
    ...
    {
      /* #50 Handle the overflow by incrementing the upper bits of compare value. */
      compareValue += Os_TimerTickType2FrtTickType(OS_OS_TICKTYPE_MAX) + 1u;
    }
    /* #60 Set the hardware compare value from corrected parameter value. */
    Os_Hal_TimerFrtSetCompareValue(TimerConfig, compareValue);                                                          /* SBSW_OS_FC_PRECONDITION */
  /* #70 otherwise: */
  }else{
...
Os_TimerIsFrtCorrectionNeeded,(void))                                                                                   /* COV_OS_HALPLATFORMFRTUNSUPPORTED_XX */
{
  return (Os_StdReturnType) (OS_OS_TICKTYPE_MAX < OS_FRT_TICKTYPE_MAX);                                                 /* PRQA S 4304 */ /* MD_Os_C90BooleanCompatibility */
}
...
/***********************************************************************************************************************
 *  Os_TimerFrtGetFrtTickTypeUpperBits()
 **********************************************************************************************************************/
...
{
  return (Value & OS_TIMERFRT_UPPERBITMASK);
}
