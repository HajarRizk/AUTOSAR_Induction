...
/**********************************************************************************************************************
  VERSION CHECK
**********************************************************************************************************************/
#if ((COM_SW_MAJOR_VERSION != 16u) || (COM_SW_MINOR_VERSION != 3u) || (COM_SW_PATCH_VERSION != 0u))
...
/*! \brief    This function checks if an invalid value has been assigned to the group signal
    ...
    \param    sduLength  Length of the received Sdu
    \return   Com_RxSigProcessState
    \context  TASK|ISR2
**********************************************************************************************************************/
LOCAL_INLINE FUNC(Com_RxSigProcessState, COM_CODE) Com_RxInv_GrpSigHasInvalidValue(Com_RxAccessInfoIterType idxRxAccessInfo, PduLengthType sduLength);
#endif
...
/*! \brief    This function is called by Com_RxIndication_Processing after an I-PDU has been received.
    ...
    \context  TASK|ISR2
**********************************************************************************************************************/
LOCAL_INLINE FUNC(boolean, COM_CODE) Com_RxIndication_Processing(PduIdType RxPduId, P2CONST(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPtr);
#endif
...
/*! \brief    This function is called to reset the Rx TP connection.
    \details  -
    \param    PduId      ID of I-PDU whose TP connection shall be reset.
    \context  TASK|ISR2
**********************************************************************************************************************/
LOCAL_INLINE FUNC(void, COM_CODE) Com_ResetRxTpConnection(PduIdType PduId);
#endif
...
**********************************************************************************************************************/
LOCAL_INLINE FUNC(BufReq_ReturnType, COM_CODE) Com_CopyRxData_Processing(PduIdType PduId, CONSTP2VAR(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPointer,
...
# if ((COM_TXTPINFOUSEDOFTXPDUINFO == STD_ON) || (COM_MINIMUMDELAYOFTXMODEINFO == STD_ON) || (COM_EXISTS_TRANSMIT_CLRUBOFTXPDUINFO == STD_ON) || (COM_CBKTXERRFUNCPTRINDUSEDOFTXPDUINFO == STD_ON))
/**********************************************************************************************************************
 ...
 *
**********************************************************************************************************************/
LOCAL_INLINE FUNC(BufReq_ReturnType, COM_CODE) Com_CopyRxData_Processing(PduIdType PduId, CONSTP2VAR(PduInfoType, AUTOMATIC, COM_APPL_DATA) PduInfoPointer, P2VAR(PduLengthType, AUTOMATIC, COM_APPL_VAR) RxBufferSizePtr)      /* PRQA S 3673 */ /* MD_MSR_Rule8.13 */
{
  /* ----- Local Variables ---------------------------------------------- */
  ...
  if(Com_IsRxPduGrpActive(PduId))       /* \trace SPEC-2736846 */
  {
    ...
    switch (Com_GetRxTpConnectionState(rxTpPduInfoIdx))
    {
      ...
      case COM_COPYING_RXTPCONNECTIONSTATE:
      {
        ...
        /* #70 Else-If RxTpConnectionState is WAITING_FOR_INDICATION */
      case COM_WAITING_FOR_INDICATION_RXTPCONNECTIONSTATE:
      {
        /* #80 If zero-sized segment is provided, do nothing, otherwise reset state machine */
...
# if (COM_TXSIGGRPINFOUSEDOFTXSIGINFO == STD_ON)
  ...
  if(Com_IsTxSigGrpInfoUsedOfTxSigInfo(idxTxSigInfo))   /* PRQA S 2842 */ /* MD_Com_2842 */ /* COV_COM_SIGGRP_CHECK */
  {
#  if ((COM_TXTPINFOUSEDOFTXPDUINFO == STD_ON) && (COM_EXISTS_UINT8_DYN_APPLTYPEOFTXSIGINFO == STD_ON))
    if(Com_IsTxTpInfoUsedOfTxPduInfo(idxTxPduInfo))     /* COV_COM_FEATURE_ALWAYS_TRUE_TX */
    {
      ...
      if(Com_GetApplTypeOfTxSigInfo(idxTxSigInfo) == COM_UINT8_DYN_APPLTYPEOFTXSIGINFO) /* COV_COM_UINT8_DYN_APPLTYPE */
      {
...
# if (COM_TXBUFFERUSEDOFTXSIGINFO == STD_ON)
    if(Com_IsTxBufferUsedOfTxSigInfo(idxTxSigInfo))     /* PRQA S 2842 */ /* MD_Com_2842 */ /* COV_COM_WRITESIGNAL_TXBUFFER */
    {
    ...
    if(Com_IsRxPduGrpActive(ComRxPduId))        /* \trace SPEC-2736846 */
    {
      ...
      switch (Com_GetRxTpConnectionState(rxTpPduInfoIdx))
      {
        ...
        case COM_READY_RXTPCONNECTIONSTATE:
        {
          ...
          if(TpSduLength <= Com_GetBufferSizeOfRxTpInfo(rxTpPduInfoIdx))
          {
          ...
          /* #60 Otherwise return BUFREQ_E_OVFL. The bufferSizePtr remains unchanged */
          else
          {
            *RxBufferSizePtr = Com_GetBufferSizeOfRxTpInfo(rxTpPduInfoIdx);     /* SBSW_COM_PTR_API */
          ...
          /* #70 Else if Com is waiting for deferred processing, reject connection. */
#  if (COM_EXISTS_DEFERRED_SIGNALPROCESSINGOFRXPDUINFO == STD_ON)
